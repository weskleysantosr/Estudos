...
@st.cache_data(ttl=300)
def load_digimons(limit: int, offset: int, level: str, type_: str, attribute: str) -> dict:
...
with tab_explore:
    ...
    page_size = 20
    result = load_digimons(limit=page_size, offset=(page - 1) * page_size, level=level, type_=type_, attribute=attribute)
    ...
    for digimon in result["items"]:
        with st.container(border=True):
            ...
            if digimon.get("image_url"):
                img_col.image(digimon["image_url"], width=80)
            info_col.subheader(digimon["name"])
            info_col.write(
